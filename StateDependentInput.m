...
duration = [1 10 20 30 40 50];


match_count = nan(length(biasval),length(act),length(duration));

for b = 1:length(duration)
    for j = 1:length(biasval)
    
        for a = 1:length(act)
            match = 0;
    
            for d = 2:3 %decision input
                for k = 1:trials
                    ...
                    for i = 1:Nt-1
                        ...
                        if any(x(i,start_node)>=act(a)) && flag==0
                            'start input';
                            noise(i,d) = noise(i,d) + biasval(j);
                            flag = 1;
                            dur = i:(i+duration(b));
                        end
                        if i==any(dur)
                    ...
                    %Check for matching decision
                    if any(x(:,2)>=neighborhood) && d==2
                        match = match+1;
                    elseif any(x(:,3)>=neighborhood) && d==3
                        match = match+1;
                    end
